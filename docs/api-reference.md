...
| `serviceDomain` _string_ | ServiceDomain is the domain actually registered with NetBird. It equals<br />spec.domain for http/tls, and the synthesized per-port subdomain<br />(<listenPort>-<mode>.<spec.domain>) for tcp/udp. |  | Optional: \{\} <br /> |
